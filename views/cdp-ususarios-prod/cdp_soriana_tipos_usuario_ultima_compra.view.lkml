view: cdp_soriana_tipos_usuario_ultima_compra {
  derived_table: {
    sql:with anioAnterior as (with rango_fecha as (
            select
--fecha inicio
format_date('%Y%m%d',DATE_SUB(DATE(max(FechaHoraTicket)), INTERVAL 365 DAY)) as  fecha_inicio,
--semana maxima del rango
cast(  format_date('%U',DATE_SUB(DATE(max(FechaHoraTicket)), INTERVAL 365 DAY)) as INT) as max_semana,
--fecha final 8 semanas antes, o 56 dias--- 90 dias
...
anioActual as (with rango_fecha as (
      select
      --fecha inicio
...
select * from anioActual
union all
select * from anioAnterior
